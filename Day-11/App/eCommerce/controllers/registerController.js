const User = require("../models/user");// ab because of this, Database ke user table se connect kar ke uspe kaam kiya ja sakta hai.
...
const register = async (req,res) => {
    ...
    try{
        const alreadyExists = await User.findOne({ where: { email }});//find if there exists a user with given email.
        if(alreadyExists){
            res.status(401).send("Email already registered");//401 -> Client Side error
        }
        else{
            // email doesn't exist in DB, to passwor le ke hash kar do.
